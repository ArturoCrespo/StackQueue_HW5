...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
        if(s == null) {
            throw new IllegalArgumentException("Illegal argument (null) String passed.") ;
        }
        ArrayListStack<Character> tempStack = new ArrayListStack<Character>() ;

        String original = s.toLowerCase().replaceAll(" ", "") ;

        for(int i = 0 ; i  < original.length(); i++) {
            tempStack.push(original.charAt(i)) ;
        }

        StringBuilder reverse = new StringBuilder();

        while(!tempStack.empty()) {
            reverse.append(tempStack.pop());
        }
